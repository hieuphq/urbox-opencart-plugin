...
class ControllerHttpauth extends Controller {
    /**
     * Function to authenticate the urbox module user
     * 
     * @return boolean
     */
    public function index() {
        ...
        $authorised = false;
        //Check for the urbox module status
        if (!$this->config->get('urbox_status')) {
            ...
            if (isset($this->request->get['SS-UserName']) && isset($this->request->get['SS-Password'])) {
                ...
                if (( $auth_user == trim($this->config->get('urbox_config_key')) ) && ( $auth_password == trim($this->config->get('urbox_verify_key')) )) {
                    $authorised = true;
                }
            }
            /**
             ...
             */
            if (!$authorised) {
                header('WWW-Authenticate: Basic realm="Urbox"');
